...
async function getCommits() {
  ...
  for (let i = 0; i < repos.length; i++) {
    let prcount = 0;
    var commits = (
      await axios.get(`https://api.github.com/repos/${repos[i]}/commits?per_page=100`)
    ).data;
    console.log("passed github");
    let p=1;
    while(commits.length){
      commits = (
        await axios.get(`https://api.github.com/repos/${repos[i]}/commits?per_page=100&page=${p}`)
      ).data;
      console.log("page = ", p);
      for(let k=0;k<commits.length; k++){
        ...
        if (message.substring(0, 10) === "Merge pull") {
          ...
          if (!isNaN(issueNumber) && name!="SomalRudra") {
            ...
            console.log(name, repos[i], points, issueNumber);
            prcount++;
          }
        }
      };
      console.log("page done = ",p);
      p++;
    }
